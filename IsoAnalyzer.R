...
if(RSEM == TRUE) {
  ...
  if(!dir.exists(rsem_base_dir)){
    ...
    if(GTF_addition == TRUE){ 
          base_dir <- "/archive/home/paolo.zarcone/Bulk-RNA/IsoAnalyzer/Output_251106"
          spladder_out_dir <- file.path(base_dir, "spladder_out") 
          
        
          gtf_source <- file.path(spladder_out_dir, "merged_sorted.gtf")
          gff3_source <- file.path(spladder_out_dir, "spladder_combined.gff3") # Il file che contiene il link ID->Gene
    ...
    python_script_path <- "/archive/home/paolo.zarcone/Bulk-RNA/IsoAnalyzer/modify_gtf.py"
